...
public class PlayerMovement : MonoBehaviour
{
	...
	private void OnTriggerEnter2D(Collider2D collision)
	{
		...
		{
			case "Chest 1":
				if (SystemVariables.playerData.chest1Opened)
				{
					DialogueTrigger.TriggerDialogue(new Dialogue{
						NPCName   = "Chest",
						sentances = new[]{"The chest is empty"}
					});
					return;
				}
				...
				DialogueTrigger.TriggerDialogue(new Dialogue{
					NPCName   = "Chest",
					sentances = new[]{$"You've got {Math.Round(Random.Range(75f, 150f), 2)} Jz. Use them in the shop"}
				});
				SystemVariables.playerData.money        += Random.Range(75f, 150f);
				SystemVariables.playerData.chest1Opened =  true;
				break;
			case "Chest 2":
				if (SystemVariables.playerData.chest2Opened)
				{
					DialogueTrigger.TriggerDialogue(new Dialogue{
						NPCName   = "Chest",
						sentances = new[]{"The chest is empty"}
				...
				break;
			case "Chest 3":
				if (SystemVariables.playerData.chest3Opened)
				{
					DialogueTrigger.TriggerDialogue(new Dialogue{
						NPCName   = "Chest",
						sentances = new[]{"The chest is empty"}
